...
const PAGES: Record<string, string> = {
  cage: "Cage status", checkouts: "Active checkouts", schedule: "Shoot schedule",
  assets: "All assets", kits: "Kits", flags: "Service flags", drift: "Kit drift log",
  untagged: "Untagged assets", badges: "Staff badges", guests: "Guest tokens",
  "kiosk-admin": "Kiosk devices", integrations: "Integrations", audit: "Audit log", billing: "Billing",
};
...
export default function DashboardPage() {
      ...
      {SIDEBAR.map(section => (
        <div key={section.section} style={{ padding: "0 10px", marginBottom: 20 }}>
          ...
          {section.items.map(item => (
            <button key={item.key} onClick={() => { setActiveKey(item.key); setMobileSidebarOpen(false); }} style={{
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100vh", overflow: "hidden" }}>
      ...
      {/* Mobile section selector — quick chips at top */}
      {isMobile && (
        <div className="scroll-x" style={{
          ...
          <button onClick={() => setMobileSidebarOpen(true)} style={{
            ...
            color: "var(--t2)", fontSize: 12, fontFamily: "'DM Sans',sans-serif",
          ...
          {[
            { key: "cage", label: "Cage" },
            { key: "checkouts", label: "Checkouts", badge: 7, badgeColor: "var(--amber)" },
            { key: "assets", label: "Assets" },
            { key: "kits", label: "Kits" },
            { key: "flags", label: "Flags", badge: 3, badgeColor: "var(--red)" },
            { key: "drift", label: "Drift", badge: 2, badgeColor: "var(--amber)" },
          ].map(c => (
            <button key={c.key} onClick={() => setActiveKey(c.key)} style={{
              ...
              {c.label}
              {c.badge && (
                <span style={{ fontFamily: "'DM Mono',monospace", fontSize: 10, color: c.badgeColor }}>{c.badge}</span>
              )}
            </button>
          ))}
        </div>
      ...
      {isMobile && mobileSidebarOpen && (
        <div onClick={() => setMobileSidebarOpen(false)} style={{
          ...
          zIndex: 100, animation: "fade-up 0.15s ease",
        }}>
          <div onClick={e => e.stopPropagation()} style={{
            position: "absolute", top: 0, left: 0, bottom: 0,
            width: 280, maxWidth: "85vw",
            background: "var(--bg)", borderRight: "1px solid var(--b1)",
            padding: "16px 0",
            paddingTop: `max(16px, var(--safe-top))`,
            paddingBottom: `max(16px, var(--safe-bottom))`,
            overflowY: "auto",
            animation: "slide-in-left 0.2s cubic-bezier(0.22,1,0.36,1)",
          }}>
      ...
      <div style={{
        ...
        gridTemplateColumns: isMobile ? "none" : "210px 1fr",
        flex: 1,
        overflow: "hidden",
      }}>
        {/* Desktop sidebar */}
        ...
        <main style={{
          ...
          <div style={{
            ...
            <div>
              <div style={{ fontFamily: "'Syne', sans-serif", fontSize: isMobile ? 18 : 20, fontWeight: 700, letterSpacing: -0.5 }}>{PAGES[activeKey]}</div>
              <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 11, color: "var(--t2)", marginTop: 3 }}>
                {isMobile ? "Updated just now" : "LMG05 · Adams · Mumford · Cellar — updated just now"}
              </div>
            </div>
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <button style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "8px 14px", borderRadius: 6, fontSize: 12, fontWeight: 500, background: "var(--acc)", color: "var(--bg)", border: "none", cursor: "pointer", fontFamily: "'DM Sans', sans-serif", minHeight: 36 }}>
                + Guest token
              </button>
            </div>
          </div>
          ...
          {(activeKey === "cage" || activeKey === "checkouts") && (
            <div style={{
              ...
              {[
                ...
                { label: "Checked out", value: STATS.checkedOut, sub: "across 2 shoots", color: "var(--amber)" },
                { label: "Service flags", value: STATS.serviceFlags, sub: `${STATS.criticalFlags} critical`, color: "var(--red)" },
                { label: "Kit drift", value: STATS.kitDriftEvents, sub: "unresolved", color: "var(--blue)" },
              ].map(s => (
                  ...
                  {CHECKOUTS.map(co => (
                    <Link key={co.id} href={`/profile/${co.initials}`} style={{ textDecoration: "none", color: "inherit" }}>
                      <div style={{
                        ...
                        opacity: co.status === "overdue" ? 0.75 : 1,
            ...
            <div className="animate-fade-up">
              ...
              <Card>
                ...
                <div style={{ padding: "10px 16px", borderTop: "1px solid var(--b1)", fontSize: 11, fontFamily: "'DM Mono', monospace", color: "var(--t3)" }}>
                  Showing {filteredAssets.length} of {ASSETS.length} loaded · 600 total
                </div>
              </Card>
            </div>
          )}
              ...
              <Card>
                {ASSETS.filter(a => a.serviceFlag).map(a => (
                  <div key={a.id} style={{ padding: "14px 16px", borderBottom: "1px solid var(--b1)", display: "flex", gap: 14, alignItems: "flex-start", flexWrap: isMobile ? "wrap" : "nowrap" }}>
                    ...
                    <button style={{ padding: "8px 14px", borderRadius: 5, fontSize: 12, background: "transparent", border: "1px solid var(--b1)", color: "var(--t2)", cursor: "pointer", fontFamily: "'DM Sans', sans-serif", minHeight: 36, whiteSpace: "nowrap" }}>Resolve</button>
                  </div>
                ))}
              </Card>
            </div>
          ...
          {activeKey === "drift" && (
            <div className="animate-fade-up">
              <Card>
                {[
                  { kit: "Venice Cinema Kit", asset: "SmallHD Monitor", type: "missing_on_return", detectedAt: "2 days ago", user: "D. Colon Jr." },
                  { kit: "Shure ULXD Kit", asset: "ULXD1 Beltpack #3", type: "missing_on_return", detectedAt: "Today", user: "B. Smith" },
                ].map((d, i) => (
