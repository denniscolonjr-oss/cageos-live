...
export default function AddShootModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  function reset() {
    setTitle(""); setClient(""); setStartsAt(""); setEndsAt(""); setLocation("");
    setLeadInitials(""); setAssignedTeam(new Set()); setAssignedKits(new Set()); setNotes("");
  }
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          <div>
            ...
            <input style={inputStyle} value={startsAt} onChange={e => setStartsAt(e.target.value)} placeholder="e.g. Tomorrow 9 AM" />
          </div>
          <div>
            <label style={labelStyle}>Ends</label>
            <input style={inputStyle} value={endsAt} onChange={e => setEndsAt(e.target.value)} placeholder="e.g. Tomorrow 4 PM" />
          </div>
        </div>

        <div>
          <label style={labelStyle}>Lead operator</label>
          <select style={inputStyle} value={leadInitials} onChange={e => setLeadInitials(e.target.value)}>
            <option value="">— No lead —</option>
            {data.profiles.map(p => (
              <option key={p.initials} value={p.initials}>{p.name} ({p.role})</option>
            ))}
          </select>
        </div>
